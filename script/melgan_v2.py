...
Original file is located at
    https://colab.research.google.com/drive/1MwsR-8vZ1ETVyqPdq8eE4nRlV1AWJOVV
"""
...
import imp
import tensorflow
imp.reload(tensorflow)
import tensorflow as tf
imp.reload(tf)
print(tf.__version__)

# !cp  /content/drive/My\ Drive/cmu_us_clb_arctic-0.95-release.zip ./ -r
# !cp  /content/drive/My\ Drive/cmu_us_bdl_arctic-0.95-release.zip ./ -r
# /content/drive/My Drive/cmu_us_bdl_arctic-0.95-release.zip
...
# !wget --header="Host: festvox.org" --header="User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.100 Safari/537.36" --header="Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3" --header="Accept-Language: it-IT,it;q=0.9,en-US;q=0.8,en;q=0.7" --header="Referer: http://festvox.org/cmu_arctic/cmu_arctic/packed/" "http://festvox.org/cmu_arctic/cmu_arctic/packed/cmu_us_ksp_arctic-0.95-release.zip" -O "cmu_us_ksp_arctic-0.95-release.zip" -c
# !unzip -qq cmu_us_ksp_arctic-0.95-release.zip #MALE1
...
shape=12              #length of time axis of split specrograms to feed to generator            
...
#MALE2
cwv = audio_array('../content/cmu_us_rms_arctic/wav')
cspec = tospec(cwv)
cdata = splitcut(cspec)
#FEMALE2
dwv = audio_array('../content/cmu_us_slt_arctic/wav')
dspec = tospec(dwv)
ddata = splitcut(dspec)
...
dsc = tf.data.Dataset.from_tensor_slices(cdata).repeat(50).map(proc, num_parallel_calls=tf.data.experimental.AUTOTUNE).shuffle(10000).batch(bs, drop_remainder=True)
dsd = tf.data.Dataset.from_tensor_slices(ddata).repeat(50).map(proc, num_parallel_calls=tf.data.experimental.AUTOTUNE).shuffle(10000).batch(bs, drop_remainder=True)
...
def deconv2d(layer_input, layer_res, filters, kernel_size=4, conc=False, scalev=False, bnorm=True, up=True, padding='same', strides=2):
  if up:
...
#   g1 = conv2d(g0, vec_len, kernel_size=(h,3), strides=1, padding='valid')
#   g2 = conv2d(g1, vec_len, kernel_size=(1,9), strides=(1,2))
#   g3 = conv2d(g2, vec_len, kernel_size=(1,7), strides=(1,2))
#   # #upscaling
...
#   return Model(inp,g3, name='E')


def mal_vec(g,iden):
  si=tuple(iden.shape.as_list())
  # iden = DenseSN(vec_len, kernel_initializer=init)(iden)
  
  iden = tf.expand_dims(iden,2)
  iden = tf.expand_dims(iden,3)
  # print(iden.shape.as_list())
  # iden = tf.reshape(iden, tuple(list(si)+[1,1]))
  s=tuple(g.shape.as_list())
  s_new=tuple([-1,s[1]*s[2],s[3]])
  g=tf.reshape(g, s_new)
  g = tf.transpose(g, [0,2,1], name='t1')
  # g = tf.reshape(g,tuple( g.shape.as_list()+[1]))
  g = tf.expand_dims(g,3)
  g= tf.matmul(g, iden)
  g = tf.transpose(g, [0,2,3,1], name='t1')
  g=tf.reshape(g, [-1,s[1],s[2],s[3]])
  return g

def stack_vec(g,iden):
  si=tuple(iden.shape.as_list())
  s=tuple(g.shape.as_list())
  # iden = DenseSN(vec_len, kernel_initializer=init)(iden)
  
  iden = tf.expand_dims(iden,1)
  iden = tf.expand_dims(iden,1)
  iden=tf.tile(input=iden, multiples=[1,s[1],s[2],1])
  # print(iden.shape.as_list())
  # iden = tf.reshape(iden, tuple(list(si)+[1,1]))
  # s=tuple(g.shape.as_list())
  # s_new=tuple([-1,s[1]*s[2],s[3]])
...
def build_generator(input_shape):
  h,w,c = input_shape
  inp = Input(shape=input_shape)
  idena = Input(shape=(vec_len,))
  idenb = Input(shape=(vec_len,))
  # si=tuple(idenb.shape.as_list())
  # idena = tf.reshape(idena, tuple(list(si)+[1,1]))
  # idenb = tf.reshape(idenb, tuple(list(si)+[1,1]))
  # idenb = tf.transpose(idenb, [0,2,1], name='t2')
  #downscaling
  g0 = tf.keras.layers.ZeroPadding2D((0,1))(inp)
  
  g1 = conv2d(g0, vec_len, kernel_size=(h,3), strides=1, padding='valid')
  # idena = DenseSN(vec_len, kernel_initializer=init)(idena)
  # idena =tf.expand_dims(idena,1)
  # idena =tf.expand_dims(idena,1)

  # idena=tf.tile(input=idena, multiples=[1,1,12,1])
  # g1 =g1 +idena
  # s=tuple(g1.shape.as_list())
  # s_new=tuple([s[0],-1,s[-1]])
  # g1=tf.reshape(g1, s_new)
  # g1 = tf.transpose(g1, [0,2,1], name='t1')
  # g1= DenseSN(vec_len, kernel_initializer=init)(g1)
  g1 = stack_vec(g1, idena)
  g2 = conv2d(g1, vec_len, kernel_size=(1,9), strides=(1,2))
  g2 = stack_vec(g2, idena)
  g3 = conv2d(g2, vec_len, kernel_size=(1,7), strides=(1,2))
  g3 = stack_vec(g3, idena)
  # upscaling
  g4 = deconv2d(g3,g2, vec_len, kernel_size=(1,7), strides=(1,2))
  g4 = stack_vec(g4, idenb)
  g4 = Concatenate()([g4 ,g2])
  
  g5 = deconv2d(g4,g1, vec_len, kernel_size=(1,9), strides=(1,2), bnorm=False)
  g5 = stack_vec(g5, idenb)
  g5 = Concatenate()([g5 ,g1])
  
  
  g6 = ConvSN2DTranspose(1, kernel_size=(h,1), strides=(1,1), kernel_initializer=init, padding='valid', activation='tanh')(g5)
  # g6 = mal_vec(g6, idenb)
  return Model([inp,idena,idenb],g6, name='G')

#Siamese Network
def build_siamese(input_shape,name):
  h,w,c = input_shape
  inp = Input(shape=input_shape)
  
  g1 = conv2d(inp, vec_len, kernel_size=(h,3), strides=1, padding='valid', sn=False)
  
  g2 = conv2d(g1, vec_len, kernel_size=(1,9), strides=(1,2), sn=False)
  
  g3 = conv2d(g2, vec_len, kernel_size=(1,7), strides=(1,2), sn=False)
  
  g4 = Flatten()(g3)
  g5 = Dense(vec_len)(g4)
  return Model(inp, g5, name='S-'+name)

#Discriminator (Critic) Network
def build_critic(input_shape):
  h,w,c = input_shape
  inp = Input(shape=input_shape)
  iden = Input(shape=(vec_len,))
  g1 = conv2d(inp, vec_len, kernel_size=(h,3), strides=1, padding='valid', bnorm=False)
  g1 = stack_vec(g1, iden)
  g2 = conv2d(g1, vec_len, kernel_size=(1,9), strides=(1,2), bnorm=False)
  g2 = stack_vec(g2, iden)
  g3 = conv2d(g2, vec_len, kernel_size=(1,7), strides=(1,2), bnorm=False)
  g3 = stack_vec(g3, iden)
  g4 = Flatten()(g3)
  g4 = DenseSN(1, kernel_initializer=init)(g4)
  return Model([inp,iden], g4, name='C')
...
def build():
  gen = build_generator((hop,shape,1))
  siam1 = build_siamese((hop,shape,1),'content')
  siam2 = build_siamese((hop,shape,1),'identity')
  ...
  return gen,critic,siam1,siam2

#Generate a random batch to display current training results
def testgena(sspec):
  sw = True
  while sw:
    a = np.random.choice(sspec)
    if a.shape[1]//shape!=1:
      sw=False
  dsa = []
  if a.shape[1]//shape>12:
    num=12
  else:
    num=a.shape[1]//shape
...
def save_test_image_full(path,epoch):
  ...
  a = testgena(sspec=cspec)
  b = testgena(sspec=dspec)
  s = min(a.shape[0],b.shape[0])
  a,b=a[:s],b[:s]
  # aa,aa2,aa3 = extract_image(a)
  # bb,bb2,bb3 = extract_image(b)
  # print (bb.shape)
  s2a = siam2(a, training=False)
  # s2a2 = siam2(b, training=False)
  s2b = siam2(b, training=False)
  # s2b2 = siam2(bb3, training=False)
  print(a.shape,s2a.shape,s2b.shape)
  ab = gen([a,s2a,s2b], training=False)
  ab = testass(ab)
  a = testass(a)
  abwv = deprep(ab)
  awv = deprep(a)
  sf.write(path+'/'+str(epoch)+'-new_file.wav', abwv, sr)
  sf.write(path+'/'+str(epoch)+'-source_file.wav', awv, sr)
...
def save_end(epoch,gloss,closs,mloss,n_save=3,save_path='../content/results/'):                 #use custom save_path (i.e. Drive '../content/drive/My Drive/')
  if epoch % n_save == 0:
    ...
    save_test_image_full(path,epoch)
#Losses
...
def loss_siamese(sa,sa1,delta=2.0):
  logits = tf.sqrt(tf.reduce_sum((sa-sa1)**2, axis=-1, keepdims=True))
  return tf.reduce_mean(tf.square(tf.maximum((delta - logits), 0)))

def loss_siamese2(sa,sa1,delta=0.0):
  logits = tf.sqrt(tf.reduce_sum((sa-sa1)**2, axis=-1, keepdims=True))
  return tf.reduce_mean(tf.square(tf.maximum((logits-delta ), 0)))
...
def get_networks(shape, load_model=False, path=None):
  ...
  return gen,critic,[siam1,siam2], [opt_gen,opt_disc]
...
def train_all(a,b):
  ...
  with tf.GradientTape() as tape_gen, tf.GradientTape() as tape_disc:
    s2a = siam2(aa, training=True)
    s2a2 = siam2(aa3, training=True)
    s2b = siam2(bb, training=True)
    s2b2 = siam2(bb3, training=True)
    print((s2b+s2b2)/2)
    #translating A to B
    fab = gen([aa,(s2a+s2a2)/2,(s2b+s2b2)/2], training=True)
    fab2 = gen([aa2,(s2a+s2a2)/2,(s2b+s2b2)/2], training=True)
    fab3 = gen([aa3,(s2a+s2a2)/2,(s2b+s2b2)/2], training=True)
    #identity mapping B to B                                                        COMMENT THESE 3 LINES IF THE IDENTITY LOSS TERM IS NOT NEEDED
    fid = gen([bb,(s2b+s2b2)/2,(s2b+s2b2)/2], training=True) 
    fid2 = gen([bb2,(s2b+s2b2)/2,(s2b+s2b2)/2], training=True)
    fid3 = gen([bb3,(s2b+s2b2)/2,(s2b+s2b2)/2], training=True)
    #concatenate/assemble converted spectrograms
    fabtot = assemble_image([fab,fab2,fab3])

    #feed concatenated spectrograms to critic
    cab = critic([fabtot,(s2b+s2b2)/2], training=True)
    cb = critic([b,(s2b+s2b2)/2], training=True)
    #feed 2 pairs (A,G(A)) extracted spectrograms to Siamese
    sab = siam1(fab, training=True)
    sab2 = siam1(fab3, training=True)
    sa = siam1(aa, training=True)
    sa2 = siam1(aa3, training=True)

    s2ab = siam2(fab, training=True)
    s2ab2 = siam2(fab3, training=True)
    

    #identity mapping loss
    loss_id = (mae(bb,fid)+mae(bb2,fid2)+mae(bb3,fid3))/3.                         #loss_id = 0. IF THE IDENTITY LOSS TERM IS NOT NEEDED
    #travel loss
    ss=tf.concat([sa2,sab2],axis=-1)
    # ss=tf.random.shuffle( ss )
    ss=tf.gather(ss, tf.random.shuffle(tf.range(tf.shape(ss)[0])))
    shuffled_sa2,shuffled_sab2=tf.split(ss, num_or_size_splits=2, axis=-1)
    shuffled_s2a2,shuffled_s2b2=tf.gather(s2a2, tf.random.shuffle(tf.range(tf.shape(s2a2)[0]))),tf.gather(s2b2, tf.random.shuffle(tf.range(tf.shape(s2b2)[0])))
    lt=loss_travel(sa,sab,shuffled_sa2,shuffled_sab2)#loss_travel(s2a,s2a2,s2ab,s2ab2)
    ls=loss_siamese(sa,shuffled_sa2)
    ls2aa=(loss_siamese2(s2a,shuffled_s2a2,delta=0)+loss_siamese2(s2b,shuffled_s2b2,delta=0))/2
    ls2ab=(loss_siamese(s2a,s2b)+loss_siamese(s2a2,s2b2))/2
    loss_m = lt+ls+ls2aa+ls2ab
  ...
  grad_gen = tape_gen.gradient(lossgtot, gen.trainable_variables+siam1.trainable_variables+siam2.trainable_variables)
  opt_gen.apply_gradients(zip(grad_gen, gen.trainable_variables+siam1.trainable_variables+siam2.trainable_variables))

  grad_disc = tape_disc.gradient(loss_d, critic.trainable_variables+siam2.trainable_variables)
  opt_disc.apply_gradients(zip(grad_disc, critic.trainable_variables+siam2.trainable_variables))
  
  return loss_dr,loss_df,loss_g,loss_id,[ls2aa,ls2ab]
...
def train(epochs, batch_size=16, lr=0.0001, n_save=6, gupt=5):
  ...
  for epoch in range(epochs):
        ...
        for batchi,(a,b,n,d) in enumerate(zip(dsa,dsb,dsc,dsd)):
            ...
            if batchi%gupt==0:
              for s in [a,b,n,d]:
                for t in [a,b,n,d]:
                  if s is not t:
                    ...
                    dloss_t,dloss_f,gloss,idloss,[ls2aa,ls2ab] = train_all(s,t)
              ...
              for s in [a,b,n,d]:
                for t in [a,b,n,d]:
                  if s is not t:
                    dloss_t,dloss_f = train_d(s,t)
              # dloss_t,dloss_f = train_d(b,a)
            ...
            if batchi%600==0:
                ...
                print(f'[a-a: {np.mean(laa[-g:])}] ', end='')
                print(f'[a-b: {np.mean(lab[-g:])}] ', end='')
                print(f'[LR: {lr}]')
                # return s2a,s2a2
...
# shape=12
gen,critic,[siam1,siam2], [opt_gen,opt_disc] = get_networks(shape, load_model=False, path='../content/drive/My Drive/MELGANVC-15-0.5410417-0.5837993-0.1247737')
...
save_test_image_full(path='../content/')

sa,sa1=s2a,s2a2
# loss_siamese(s2a,s2a2).numpy()
logits = tf.sqrt(tf.reduce_sum((sa-sa1)**2, axis=-1, keepdims=True))
tf.reduce_mean(tf.square(tf.maximum((logits-delta  ), 0))).numpy()

!cp /content/MELGANVC-0.1919441-0.6837348-0.1870702 /content/drive/My\ Drive -r
