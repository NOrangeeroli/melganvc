...
def build_siamese2(input_shape):
  h,w,c = input_shape
  inp = Input(shape=input_shape)
  g1 = conv2d(inp, 256, kernel_size=(h,3), strides=1, padding='valid', sn=False)
  g2 = conv2d(g1, 256, kernel_size=(1,9), strides=(1,2), sn=False)
...
def build():
  gen = build_generator((hop,shape,1))
  siam = build_siamese((hop,shape,1))
  siam2 = build_siamese2((hop,shape,1))
...
def save_test_image_full(path,epoch):
  ...
  sf.write(path+'/'+str(epoch)+'-recon_file.wav', bbwv, sr)
...
def get_networks(shape, load_model=False, path=None):
  if not load_model:
    gen,critic,siam, siam2= build()
  else:
    gen,critic,siam,siam2 = load(path)
  print('Built networks')

  opt_gen = Adam(0.0001, 0.5)
  opt_disc = Adam(0.0001, 0.5)

  return gen,critic,[siam,siam2], [opt_gen,opt_disc]
...
def train_all(a,b):
  ...
  with tf.GradientTape() as tape_gen, tf.GradientTape() as tape_disc:
    ...
    #travel loss
    loss_m = loss_travel(sa,sab,sa2,sab2)+loss_siamese(sa,sa2)+loss_travel(s2a,s2a2,s2ab,s2ab2)+loss_siamese(s2a,s2ab)
  ...
  grad_gen = tape_gen.gradient(lossgtot, gen.trainable_variables+siam.trainable_variables+siam2.trainable_variables)
  opt_gen.apply_gradients(zip(grad_gen, gen.trainable_variables+siam.trainable_variables+siam2.trainable_variables))
...
# shape=6
gen,critic,[siam,siam2], [opt_gen,opt_disc] = get_networks(shape, load_model=False, path='../content/drive/My Drive/')
